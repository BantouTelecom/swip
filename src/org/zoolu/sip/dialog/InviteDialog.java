...
{  
   ...
   {  printLog("inside invite(callee,caller,contact,sdp)",LogLevel.MEDIUM);
      ...
      Message invite=msgf.createInviteRequest(sip_provider,request_uri,to_url,from_url,contact_url,session_descriptor);
      // do invite
      invite(invite);
   }
   ...
   {  printLog("inside reInvite(contact,sdp)",LogLevel.MEDIUM);
      ...
      // else
      MessageFactory msgf = new MessageFactory();
      Message invite=msgf.createInviteRequest(this,session_descriptor);
      if (contact!=null)
   ...
   {  if (contact!=null) setLocalContact(new NameAddress(contact));
   	  MessageFactory msgf = new MessageFactory();   
      Message ack=msgf.create2xxAckRequest(this,session_descriptor);
      ackWithAnswer(ack);
   }
   ...
   {  
	   Logger.paranoia("InviteDialog.respond()");
      ...
      {  if (!verifyStatus(statusIs(D_INVITED)||statusIs(D_ReINVITED)))
         {  Logger.paranoia("respond(): InviteDialog not in (re)invited state: No response now");
            return;
         }
   ...
   public void ring()
   {  
	  Logger.paranoia("InviteDialog.ring()");
   ...
   {  printLog("inside bye()",LogLevel.MEDIUM);
      ...
      {  
    	 MessageFactory msgf = new MessageFactory(); 
    	 Message bye=msgf.createByeRequest(this);
         bye(bye);        
      }
   }
   ...
   {  printLog("inside cancel()",LogLevel.MEDIUM);
      ...
      {  
    	 MessageFactory msgf = new MessageFactory();
    	 Message cancel=msgf.createCancelRequest(invite_req);
         cancel(cancel);
      }
      else
   ...
   {  printLog("inside onTransSuccessResponse(tc,msg)",LogLevel.LOW);
      ...
      {  if (!verifyStatus(statusIs(D_INVITING)||statusIs(D_ReINVITING))) return;
         ...
         {  //invite_req=MessageFactory.createRequest(SipMethods.ACK,dialog_state,sdp.toString());
            ...
            ack_req=msgf.create2xxAckRequest(this,null);
            AckTransactionClient ack_tc=new AckTransactionClient(sip_provider,ack_req,null);
            ack_tc.request();
         }
         if (!re_inviting)
