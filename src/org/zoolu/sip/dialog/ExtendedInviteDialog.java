...
{  
   ...
   {  
	   ...
	   Logger.paranoia("ExtendedInviteDialod constructor#1");	   
      init(listener);
      this.username=provider.getUserProfile().username;
      this.realm=provider.getUserProfile().realm;
      this.passwd=provider.getUserProfile().passwd;
      authId=provider.getUserProfile().authID;
   }
   ...
   {  super(provider,listener);
   		Logger.paranoia("ExtendedInviteDialod constructor#2");   
   		init(listener);
   ...
   {  this.dialog_listener=listener;
   		if (this.dialog_listener!=null)Logger.paranoia("       dialog_listener="+this.dialog_listener);
   ...
   {  
	  ...
	  Message req=msgf.createReferRequest(this,refer_to,referred_by);
      request(req);
   }
   ...
   {  
	  MessageFactory msgf = new MessageFactory(); 
	  Message req=msgf.createNotifyRequest(this,"refer",null,sipfragment);
      request(req);
   }
   ...
   {  printLog("inside acceptRefer(refer)",LogLevel.MEDIUM);
      Message resp=MessageFactory.createResponse(req,202,SipResponses.reasonOf(202),null);
      respond(resp);
   } 
   ...
   {  
	   Logger.paranoia("ExtendedInviteDialog.onReceivedMessage");
   ...
   {  Logger.paranoia("inside onTransFailureResponse");
      ...
      Logger.paranoia("ExtendedInvi;teDialog.onTransFailureResponse:");
      Logger.paranoia("attempts (max) ="+attempts+" ("+MAX_ATTEMPTS+")");
      Logger.paranoia("has authenticate header? "+msg.hasProxyAuthenticateHeader());
      Logger.paranoia("realm="+realm);
      Logger.paranoia("msg realm="+((msg.hasProxyAuthenticateHeader()) ? msg.getProxyAuthenticateHeader().getRealmParam() : "n/a "));
      ...
      Message req=tc.getRequestMessage();
         ...
         // Lähetämme uuden inviten, joten teemme uuden invitetransactionclientin, eikä transaction clientia
