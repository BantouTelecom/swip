...
{  
   ...
   {  
	  ...
	  Message req=msgf.createNotifyRequest(this,event,id,null,null);
   ...
   {  
	  MessageFactory msgf = new MessageFactory();
	  Message req=msgf.createNotifyRequest(this,event,id,content_type,body);
      if (state!=null)
   ...
   {  
	   Logger.debug("NotifierDialog.onReceivedMessage");
      if (statusIs(D_TERMINATED))
      ...
      {  if (statusIs(this.D_WAITING))
         {  // the first SUBSCRIBE request
            changeStatus(D_SUBSCRIBED);
            Logger.debug("NotifierDialog.onReceivedMessage deleting SUBSCRIBE listeners");
