...
public abstract class BaseMessageFactory
{
	...
	private String uaInfoBase = null;
	private String uaInfoExt = null;
   ...
   public Message createRequest(String method, SipURL request_uri, NameAddress to, NameAddress from, NameAddress contact, String proto, String via_addr, int host_port, boolean rport, String call_id, long cseq, String local_tag, String remote_tag, String branch, String body)
   {  boolean addPrivacyHeaders=false;
      ...
      if(sipProvider !=null && sipProvider.getUserProfile().requestPrivacy==true)
      {
    	  //if (method.equals(SipMethods.INVITE))
    	  if (true)
    			  {
    		  		req.setFromHeader(new FromHeader(NameAddress.anonymous(),local_tag));
    		  		addPrivacyHeaders=true;
    		  	  }
      }
      ...
      // SESCA removed due to buggy nokia e-series
      //req.setExpiresHeader(new ExpiresHeader(String.valueOf(SipStack.default_expires)));
      // add User-Agent header field
      if (getUaInfo()!=null) req.setUserAgentHeader(new UserAgentHeader(getUaInfo()));
      if (addPrivacyHeaders)
      {
    	  req.setPrivacyHeader(new PrivacyHeader("id"));
    	  req.setPPreferredIdentityHeader(new PPreferredIdentityHeader (from));
      }
   ...
   public Message createRequest(SipProvider sip_provider, String method, SipURL request_uri, NameAddress to, NameAddress from, NameAddress contact, String call_id, long cseq, String local_tag, String remote_tag, String branch, String body)
...
/**
	 ...
	 * @see #createRequest(String,SipURL,NameAddress,NameAddress,NameAddress,String,String,int,String,long,String,String,String,String)
	 */
	public Message createRequest(SipProvider sip_provider, String method, SipURL request_uri, NameAddress to, NameAddress from, NameAddress contact, String body)
	 ...
	 * <LI> transport protocol is taken from request-uri (if transport parameter
	 * is present) or the default transport for the SipProvider is used.
	 * <LI> if no callId given it is generated
	 * <LI> cseq is picked random
	 * <LI> local_tag is picked random
	 * <LI> branch is picked random
	 * </UL>
	 ...
	 * @see #createRequest(String,SipURL,NameAddress,NameAddress,NameAddress,String,String,int,String,long,String,String,String,String)
	 */
	public Message createRequest(SipProvider sip_provider, String method, SipURL request_uri, NameAddress to, NameAddress from, NameAddress contact, String body, String callId, String remoteTag)
	{ // SipURL request_uri=to.getAddress();
		if(callId == null)
			callId = sip_provider.pickCallId();
		
		int cseq = SipProvider.pickInitialCSeq();
		String local_tag = SipProvider.pickTag();
		// String branch=SipStack.pickBranch();
		setUaInfo(sip_provider);		
		return createRequest(sip_provider, method, request_uri, to, from, contact, callId, cseq, local_tag, null, null, body);
	}
	 ...
	 * <LI> if no callId given it is generated
	 * <LI> cseq is picked random
	 * <LI> if no local_tag given it is generated
	 * <LI> branch is picked random
	 * </UL>
	 * 
	 * @see #createRequest(String,SipURL,NameAddress,NameAddress,NameAddress,String,String,int,String,long,String,String,String,String)
	 */
	public Message createRequest(SipProvider sip_provider, String method, SipURL request_uri, NameAddress to, NameAddress from, NameAddress contact, String body, String callId, String localTag, String remoteTag)
	{ // SipURL request_uri=to.getAddress();
		if(callId == null)
			callId = sip_provider.pickCallId();
		...
		String local_tag;
		if(localTag == null)
			local_tag = SipProvider.pickTag();
		else
			local_tag = localTag;
		// String branch=SipStack.pickBranch();
		setUaInfo(sip_provider);		
		return createRequest(sip_provider, method, request_uri, to, from, contact, callId, cseq, local_tag, remoteTag, null, body);
	}
   ...
   public Message createRequest(Dialog dialog, String method, String body)
   {  
	  setUaInfo(dialog.getSipProvider()); 
	  NameAddress to=dialog.getRemoteName();
      NameAddress from;
	  if (sipProvider.getUserProfile().requestPrivacy==false)
    	  from=dialog.getLocalName();
	  else
		  from = NameAddress.anonymous();
   ...
   public Message createInviteRequest(SipProvider sip_provider, SipURL request_uri, NameAddress to, NameAddress from, NameAddress contact, String body)
   {  String call_id=sip_provider.pickCallId();
      int cseq=SipProvider.pickInitialCSeq();
      String local_tag=SipProvider.pickTag();
      //String branch=SipStack.pickBranch();
   ...
   public Message createInviteRequest(Dialog dialog, String body)
   { 
	   setUaInfo(dialog.getSipProvider());
   ...
   /** Creates an ACK request for a 2xx response.
     * @see #createRequest(Dialog,String,String) */
   public Message create2xxAckRequest(Dialog dialog, String body)
   {  
	   setUaInfo(dialog.getSipProvider());
   ...
   public Message createNon2xxAckRequest(SipProvider sip_provider, Message method, Message resp)
   {  
	  setUaInfo(sip_provider); 
   ...
   public Message createCancelRequest(Message method)
   ...
   public Message createByeRequest(Dialog dialog)
   {  
	  setUaInfo(dialog.getSipProvider());
   ...
   public Message createRegisterRequest(SipProvider sip_provider, NameAddress to, NameAddress from, NameAddress contact)
   {  setUaInfo(sip_provider);
      ...
      req.setAllowHeader(new AllowHeader("INVITE, ACK, BYE, CANCEL, REFER"));
      return req;
   }
   ...
   private void setUaInfo(SipProvider sip_provider) {
	   if (sipProvider!=null) return;
	   sipProvider=sip_provider;
	   uaInfoBase=sip_provider.UABase;
	   uaInfoExt=sip_provider.UAIdentity;
	}
   private String getUaInfo() {
	   // test: return "Sipura/SPA941-4.1.8";

	   if (uaInfoBase!=null && uaInfoExt != null && uaInfoBase.length()>0 && uaInfoExt.length()>0)
		   return uaInfoBase+"-"+uaInfoExt;
	   else return SipStack.ua_info;
