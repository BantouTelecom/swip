   ...
   {  
      ...
      {  int code=msg.getStatusLine().getCode();
         ...
         {  if (statusIs(STATE_TRYING) || statusIs(STATE_PROCEEDING))
            {  
        	   Logger.paranoia("InviteTransactionClient.onReceivedMessage, status="+status);
        	   retransmission_to.halt();
               transaction_to.halt();
               MessageFactory msgf = new MessageFactory();
               ack=msgf.createNon2xxAckRequest(sip_provider,request,msg);
               changeStatus(STATE_COMPLETED);
               if (transaction_listener!=null) transaction_listener.onTransFailureResponse(this,msg);
               Logger.paranoia("InviteTransactionClient.onReceivedMessage: transaction_listener="+ ((transaction_listener!=null) ? transaction_listener.getClass():"null"));
            ...
            {  // retransmit ACK only in case of unreliable transport 
               Logger.paranoia("InviteTransactionClient.onReceivedMessage, status="+status+", sending ACK.");
