...
{     
   ...
   {  super(sip_provider);
      Logger.paranoia("InviteTransactionServer constructor#1");
   ...
   {  super(sip_provider);
   	  Logger.paranoia("InviteTransactionServer constructor#2");   
   ...
   {  super(sip_provider);
      Logger.paranoia("InviteTransactionServer constructor#3");
      request=new Message(invite);
      init(listener,request.getTransactionId(),request.getConnectionId());      
   ...
   {  
	  Logger.paranoia("InviteTransactionListener.init");
	  this.transaction_listener=listener;
      this.transaction_id=transaction_id;
      this.connection_id=connection_id;
      if (this.transaction_listener!=null)Logger.paranoia("       transaction_listener="+this.transaction_listener.toString()+"("+this.transaction_listener.getClass()+")");
      if (this.transaction_id!=null)Logger.paranoia("       transaction_id="+this.transaction_id.toString()+"("+this.transaction_id.getClass()+")");
      if (this.connection_id!=null)Logger.paranoia("       connection_id="+this.connection_id.toString()+"("+this.connection_id.getClass()+")");      
   ...
   public void listen()
   {  
	   Logger.paranoia("InviteTransactionServer.listen");
   ...
   {  response=resp;
   	Logger.debug("InviteTransactionServer.respondWith: status="+status);
   	Logger.debug("       Message="+resp.getFirstLine());
   ...
   {
	   Logger.paranoia("InviteTransactionServer.onReceivedMessage");
	   Logger.paranoia("       message="+msg.getFirstLine());
	   Logger.paranoia("       status="+status);
	   if (msg.isRequest())
      {  String req_method=msg.getRequestLine().getMethod();
         ...
         {
        	 Logger.paranoia("Check if message is from proxy");
        	 IpAddress pip=null;
			try {
				pip = IpAddress.getByName(provider.getOutboundProxy().getAddress().toString());
        	 ...
        	 IpAddress vip = null;
        	 try {
				vip = IpAddress.getByName(host);
			} catch (UnknownHostException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			Logger.paranoia("pip="+pip);
			Logger.paranoia("vip="+vip);
			Logger.paranoia("host="+host);
			if (!(provider.getUserProfile().allowOutsideProxyConnections) && (vip==null || pip==null || !(vip.toString().equals(pip.toString()))))
				{
				Logger.info("INVITE is not from proxy. Discarding request");
				return;
				}
            ...
            {  request=new Message(msg);
               ...
               if (auto_trying)
               {  Message trying100=MessageFactory.createResponse(request,100,SipResponses.reasonOf(100),null);
               	  Logger.paranoia("Autosend TRYING");
                  respondWith(trying100); // this method makes it going automatically to STATE_PROCEEDING
               }
               if (transaction_listener!=null) transaction_listener.onTransRequest(this,msg);
               Logger.paranoia("       transaction_listener="+transaction_listener);
               return;            
            }
   ...
   {  
	  Logger.paranoia("InviteTransactionServer.terminate");
	   retransmission_to.halt();
      clearing_to.halt();     
      end_to.halt();
