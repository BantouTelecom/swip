...
public class RtpSocket
{
   ...
   {  DatagramPacket datagram=new DatagramPacket(rtpp.packet,rtpp.packet.length);
      //socket.setReceiveBufferSize(rtpp.packet_len);   
      //System.out.println("************************* receivebffer size="+rtpp.packet_len);
   	Logger.hysteria("RtpSocket.receive: Trying");	  
   ...
   {  
      ...
      //socket.setSendBufferSize(rtpp.packet_len);
      //System.out.println("************************* sendbuffer size="+rtpp.packet_len);
      socket.send(datagram);
   	  Logger.hysteria("RtpSocket.send");
   }

   /** Closes this socket */      
   public void close()
   {  
	   Logger.info("RtpSocket.close()");
	   socket.close();
   }
